...
长按下键连接云端；长按上键打开手机局域网配网页，双击确定关闭热点但保留路由器连接。已保存的 Wi-Fi 开机自动重连，对话上方显示网络状态。Wi-Fi 使用相同的有界媒体帧，经 WSS 传输，使用 ESP-IDF CA 证书包核验服务器，并先同步时间。云端媒体桥不保存原始录音。

每台设备由部署者通过 USB 的 `network.configure` 注入独立设备凭证，保存到 NVS，不写入公开固件。语音识别、合成服务密钥、允许的 Agent 铭牌和执行电脑绑定保存在云端。设备凭证与远端发送凭证分离。远端播报先发送 `remote.begin`，只有设备空闲并回复 `remote.ready` 后才播放。调用者需要查云端消息回执，HTTP 接收不代表播报完成。这一增量需要单独验收 Wi-Fi、仅电池供电、远端播报和断线重连，原来的 USB 结果不能代替。
